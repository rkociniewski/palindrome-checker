#!/bin/bash
# Advanced version check for projects
...
get_version_info() {
    local file=$1
    local version_name=$(grep -E 'version\s*=\s*["\047][^"\047]+["\047]' "$file" | grep -oE '["\047][^"\047]+["\047]' | tr -d '"' | tr -d "'" | head -1)
    echo "$version_name"
}
...
# Get current version
IFS='|' read -r CURRENT_VERSION <<< "$(get_version_info "$BUILD_FILE")"
...
if [[ $BRANCH_NAME =~ ^(main|release/) ]]; then
    ...
    if git rev-parse "origin/$BRANCH_NAME" >/dev/null 2>&1; then
        ...
        if [ -n "$REMOTE_BUILD_FILE_CONTENT" ]; then
            ...
            IFS='|' read -r REMOTE_VERSION <<< "$(get_version_info "$TEMP_FILE")"
            rm "$TEMP_FILE"
            ...
            # Check versionCode
            if [ "$CURRENT_VERSION" -le "$REMOTE_VERSION" ]; then
                echo -e "${RED}❌ ERROR: versionCode was not incremented!${NC}"
                echo "   Remote: $REMOTE_VERSION"
                echo "   Local:  $CURRENT_VERSION"
                echo ""
                echo "   New versionCode must be: $((REMOTE_VERSION + 1)) or higher"
                echo ""
                exit 1
            else
                CODE_DIFF=$((CURRENT_VERSION - REMOTE_VERSION))
